...
def is_candidate(path: str, cfg: Config) -> Tuple[bool, str]:
    ...
    if video_codec in cfg.skip_codecs_video:
        return False, f"already_optimal_video ({video_codec})"
    ...
    if audio:
        ...
        if video_codec in cfg.skip_codecs_video and audio_codec in cfg.skip_codecs_audio:
            if audio_channels <= 2 or "stereo" in audio_layout:
                return False, f"already_optimal_both ({video_codec}/{audio_codec})"

    return True, "candidate"


def scan_library(cfg: Config) -> Dict:
