...
# ------------------------------------------------------------------------------
# Auto-detect container tool: prefer Podman if available (even if docker
# is a podman emulation wrapper)
# ------------------------------------------------------------------------------
if command -v podman &>/dev/null; then
	CONTAINER_TOOL="podman"
elif ! command -v docker &>/dev/null; then
	echo "[docker-run] Error: neither docker nor podman found." >&2
	exit 1
fi
...
if [[ "$CONTAINER_TOOL" == "podman" ]]; then
	...
	# Rootless podman often needs host networking on some hosts
